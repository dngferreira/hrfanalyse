...
def lzma_compress(inputfile,level,decompress):
    ...
    with open(inputfile,"r") as fdorig:
        origlines=fdorig.readlines()
    origtext = ''.join(origlines)
    compressedtext = lzma.compress(origtext)#this is a backported version of this module no levels implemneted ,preset=int(level) will be available in python3
    with open(inputfile+".lzma","w") as fdout:
        fdout.write(compressedtext)
    compressed_size = int(os.stat(inputfile+'.lzma').st_size)

    decompress_time=None
    if decompress:
        decompress_time = timeit.timeit(lambda: lzma.decompress(compressedtext),number=10)
...
def bzip2_compress(inputfile,level,decompress):
    ...
    with open(inputfile,"r") as fdorig:
        origlines=fdorig.readlines()
    origtext = ''.join(origlines)
    compressedtext = bz2.compress(origtext,int(level))
    ...
    decompress_time=None
    if decompress:
        decompress_time = timeit.timeit(lambda: bz2.decompress(compressedtext),number=10)
