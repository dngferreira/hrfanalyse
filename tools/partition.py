...
def initial_indexes_time(lines, starting_point, section, start_at_end, cumulative, time_stamp):
    ...
    while test_time_limit(cumulative, float(time), time_stamp, time_elapsed, starting_point) and abs(aux_index) < len(lines):
        if start_at_end:
            aux_index -=1
        else:
            aux_index +=1
    ...
    while test_time_limit(cumulative, float(time), time_stamp, time_elapsed, starting_point+section) and abs(aux_index) < len(lines):
        if start_at_end:
            aux_index -=1
        else:
            aux_index +=1
...
def add_parser_options(parser, full_file_option=True):
    ...
    parser.add_argument("-g", "--gap", dest="gap", metavar="SECONDS", type=float, action="store", help="gap between sections", default=0)
